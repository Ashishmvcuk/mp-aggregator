...
Each run also writes **`output/scrape_meta.json`** (timestamps, run id, versions). **`scrapedAt`** is the **completion** instant when metadata is written (data ready for the site); **`runStartedAt`** is pipeline start (matches **`run_summary.run_timestamp`**). **`main.py`** copies it to **`website/public/data/scrape_meta.json`** when website sync is enabled (same as **`sync_to_website.py`**) so the UI “Last run” stays aligned with the finished scrape.
